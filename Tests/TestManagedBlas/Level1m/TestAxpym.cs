...
public class TestAxpym
{
    ...
    // Dense + NoTrans
    [TestMethod]
    public void TestDenseNoTransRowMajor() => RunCase(DiagType.NonUnit, UpLo.Dense, TransType.NoTrans, 0.75,
    CreateMatrixRandom(rows, cols), CreateMatrixRandom(rows, cols));
    [TestMethod]
    public void TestDenseNoTransColMajor() => RunCase(DiagType.NonUnit, UpLo.Dense, TransType.NoTrans, 0.75,
    CreateMatrixTransRandom(rows, cols), CreateMatrixTransRandom(rows, cols));
    [TestMethod]
    public void TestDenseNoTransStride() => RunCase(DiagType.NonUnit, UpLo.Dense, TransType.NoTrans, 0.75,
    CreateMatrixStrideRandom(rows, cols, stride), CreateMatrixStrideRandom(rows, cols, stride + 1));

    // Dense + Trans
    [TestMethod]
    public void TestDenseTransRowMajor() => RunCase(DiagType.NonUnit, UpLo.Dense, TransType.OnlyTrans, -1.25,
    CreateMatrixRandom(cols, rows), CreateMatrixRandom(rows, cols));
    [TestMethod]
    public void TestDenseTransColMajor() => RunCase(DiagType.NonUnit, UpLo.Dense, TransType.OnlyTrans, -1.25,
    CreateMatrixTransRandom(cols, rows), CreateMatrixTransRandom(rows, cols));
    [TestMethod]
    public void TestDenseTransStride() => RunCase(DiagType.NonUnit, UpLo.Dense, TransType.OnlyTrans, -1.25,
    ...
    private static void RunCase(DiagType aDiag, UpLo aUplo, TransType aTrans, double alpha, MatrixView A, MatrixView BIn)
    {
        ...
        BlasProvider.Axpy(aDiag, aUplo, aTrans, alpha, A, Bm);
        BlisProvider.Axpy(aDiag, aUplo, aTrans, alpha, A, Be);
        var diff = Be - Bm;
